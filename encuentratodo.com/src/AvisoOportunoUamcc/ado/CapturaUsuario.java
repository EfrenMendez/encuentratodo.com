...
import AvisoOportunoUamcc.ado.conexion;
	...
	private void inicializar() {
	   ...
	   public String guardaUsuario(UsuarioTO to){
			try{
			    ...
			    String sql = "insert into usuarios(idusuario,nombre,apellido,email,usuario,contrasenia) "
			    		+ "values ('0','"+to.getNombre()+"','"+to.getApellido()+"','"+to.getEmail()+"','"+to.getUsuario()+"','"+to.getContrasenia()+"'); ";
					
			  	CallableStatement callableStatement = connection.prepareCall(sql);
			    callableStatement.execute();
				FacesContext context = FacesContext.getCurrentInstance();
				context.addMessage(null, new FacesMessage("Bienvenido "+to.getNombre()));
				return " ";
		...
		public Integer VerificaRegistroUsuario(String nombre,String Alias){
			Statement stmt = null;
			...
			try{
				try {
				    ...
				    String sql1="SELECT COUNT(*) FROM `usuarios` WHERE `nombre`='"+nombre+"' ;";
